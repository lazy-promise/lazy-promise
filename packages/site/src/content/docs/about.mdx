...
Apart from the need for AbortController plumbing, the native promise API is quite elegant, and the LazyPromise API does not just resemble it, but follows all its subtleties unless stated otherwise in the docs. This has a side benefit of making the library much easier to document and learn.
