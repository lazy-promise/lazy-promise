...
If you don't plan to use type-safe errors, feel free to skip this page. The feature only manifests itself when there is `ErrorBox` class involved. Just know that when you subscribe to a LazyPromise that's generically typed and so can technically resolve with an `ErrorBox`, you can avoid a type error by passing `any` as a type parameter: `.subscribe<any>(...)`.
