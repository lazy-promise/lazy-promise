...
Error boxes are part of what makes LazyPromise simple. Although `LazyPromise<"value" | ErrorBox<"error">>` is more verbose than `LazyPromise<"value", "error">`, an extra channel and type parameter would have introduced unnecessary complexity when it comes to using LazyPromise together with native promises and generator syntax. You wouldn't be able to produce typed errors in native async functions by returning error boxes, and try/catch/finally syntax in generator functions would have non-obvious behavior. Also, LazyPromise does not attempt to collapse types like `ErrorBox<"a"> | ErrorBox<"b">` into `ErrorBox<"a" | "b">` or to enforce a specific order of elements in the union. Large TS types have always been hard to read, and there is little LazyPromise can do about that: instead it focuses on correctness and simplicity.
