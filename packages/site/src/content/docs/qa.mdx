...
- This enables a [pattern](/recipes/#delaying-a-lazy-promise) `lazyPromise.finally(() => anotherLazyPromise)`.
...
LazyPromise is designed in such a way that all boxed errors must be either explicitly caught with `catchBoxed` or whitelisted when you call `subscribe`. `allSettled` implicitly catches errors by turning them into values that you can potentially discard without the type system flagging it.

## Why not have an equivalent of `Promise.withResolvers`?

A native promise has a single `resolve, reject` pair, whereas a LazyPromise has a new `sink` for each subscription.
...
Error boxes are part of what makes LazyPromise simple. `LazyPromise<"value" | ErrorBox<"error">>` is more verbose than `LazyPromise<"value", "error">`, but an extra channel and type parameter (or similarly, resolving promises with an object with two type parameters) would have introduced unnecessary complexity when it comes to using LazyPromise together with native promises and generator syntax. You wouldn't be able to produce typed errors in native async functions by returning error boxes, and try/catch/finally syntax in generator functions would have non-obvious behavior. Also, LazyPromise does not attempt to collapse types like `ErrorBox<"a"> | ErrorBox<"b">` into `ErrorBox<"a" | "b">` or to enforce a specific order of elements in the union. Large TS types have always been hard to read, and there is little LazyPromise can do about that: instead it focuses on correctness and simplicity.
