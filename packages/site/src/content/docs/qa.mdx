...
To avoid the overhead of creating and garbage-collecting a function. Here's how you can build a `toDisposeFn` utility that subscribes and returns a teardown function when you call `.pipe(toDisposeFn)`:
