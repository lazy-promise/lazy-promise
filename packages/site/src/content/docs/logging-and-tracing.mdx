...
A `Span` can also have a method `run(work, depth)` which wraps synchronous work done on behalf of the subscription (running the producer, the consumer or the teardown logic). `depth` is the number of `run` calls that logically enclose the work, counting this one, based on causality and until an async boundary is crossed; it is what `log` turns into dots. Physical nesting of `run` calls does not follow `depth`. A `run` implementation should therefore set up its state from `depth` rather than from the state of an enclosing `run`.
