...
There is a function `fromEager` that converts an async function to a LazyPromise, and a method `toEager` that converts a LazyPromise to a Promise. Both support the AbortController API.
...
<details>
<summary><strong>Why does <code>finally</code> not run when the lazy promise is canceled?</strong></summary>
...
- This enables the pattern `lazyPromise.finally(() => anotherLazyPromise)`, which is the equivalent of the native
  ...
  For example, `lazyPromise.finally(inMicrotask)` makes a lazy promise fire in a microtask like a native promise.
